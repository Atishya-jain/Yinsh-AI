...
void player::make_next_move(vector<vector<pos>>& board, vector<int>& moves){
	if(num_rings_placed < num_rings){
		...
		if(all_valid_moves.size() > 0){
			...
			moves.push_back(movet1);
			moves.push_back(coord1);
			moves.push_back(coord2);
			moves.push_back(movet2);
			moves.push_back(coord3);
			moves.push_back(coord4);
			my_ring_pos[ring].first = coord3.coord;
			my_ring_pos[ring].second = coord4.coord;
			remove_trails_and_rings(board, moves);
		}
		else{
...
int startX,startY,endX,endY, dir;
	...
	if(my_turn){
		...
		for(int i = 0; i<len;i++){
			bool a1 = (my_trails[dir].first).first == x1;
			bool a2 = (my_trails[dir].first).second == y1;
			bool a3 = (my_trails[dir].second).first == x2;
			bool a4 = (my_trails[dir].second).second == y2;
			if(a1 && a2 && a3 && a4){
		...
		for(int i = 0; i<len;i++){
			bool a1 = (opp_trails[dir].first).first == x1;
			bool a2 = (opp_trails[dir].first).second == y1;
			bool a3 = (opp_trails[dir].second).first == x2;
			bool a4 = (opp_trails[dir].second).second == y2;
			if(a1 && a2 && a3 && a4){
...
void player::check_my_trail(vector<vector<pos>>& board, int x1, int y1, int dir, bool my_turn){
	...
	for(int i = 0, j = 0; (i< board_size) && (j < board_size); i=i+step_x, j=j+step_y){
