...
public:
	...
	void update_board(vector<vector<pos>>& board, int action_on_ring, int initial_x, int initial_y, int final_x, int final_y, bool my_turn);
	void flip_markers(vector<vector<pos>>& board, int x1, int y1, int x2, int y2, bool my_turn);
	void remove_markers(vector<vector<pos>>& board, int start_x, int start_y, int end_x, int end_y, bool my_turn);
